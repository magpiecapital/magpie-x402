...
| [11-full-lifecycle-leverage-agent.ts](./11-full-lifecycle-leverage-agent.ts) | 0.001 SOL × arms + borrow | `build-borrow` (V4) → `self-limit-close/arm` → `loan/by-pda` | **Flagship** — one agent runs a whole leveraged-position lifecycle via the `@magpieloans/magpie-agent` SDK: borrow on V4, arm in-vault take-profit + stop-loss, inspect vault state, repay. Self-signed, non-custodial. |
| [12-rwa-carry-agent.ts](./12-rwa-carry-agent.ts) | 0.001 SOL × arms + borrow | `collateral/eligible` → `build-borrow` (V4) → `self-limit-close/arm` | **RWA carry** — borrow SOL against tokenized equity and arm an in-vault stop-loss: a no-taxable-sale carry trade. RWA borrow is launch-gated, so it degrades gracefully (prints the routing it *would* take) until the V3 equity-track launch. |
| [13-basewallet-agent.ts](./13-basewallet-agent.ts) | free reads (write paths shown) | `new MagpieAgent({ signer })` | **Bring-your-own-wallet** — drive Magpie from a SendAI / Solana Agent Kit `BaseWallet`, Privy, Turnkey or any embedded wallet via a ~10-line `MagpieSigner` adapter. No raw keypair; with a `signer` the SDK touches no secret-key bytes. Non-custodial by construction. |
